...
// Settings a recruiter has any business reading. The settings screen is open to
// everyone (it is where they change their own password), and the full row
// carries the company bank account, the GSTIN and the SMTP username — half of a
// mail credential. None of that belongs on a screen opened to see the
// replacement window, so the read is narrowed by role rather than masked.
const OPEN_TO_EVERYONE = [
  "id", "companyName", "replacementDays", "defaultPaymentDays",
  "careersEnabled", "careersIntro", "careersEmail", "updatedAt",
];

export function settingsForRole(s, canSeeAll) {
  const full = publicSettings(s);
  if (canSeeAll) return full;
  const out = {};
  for (const k of OPEN_TO_EVERYONE) if (k in full) out[k] = full[k];
  return out;
}
