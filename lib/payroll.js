...
/**
 * How many CALENDAR days of this month this person was actually on the payroll.
 *
 * A full month for anybody who was here throughout — which is everybody, until
 * somebody fills in a date. That equivalence is the point: with both dates null
 * this returns daysInMonth and every figure below reduces exactly to what it
 * was before pro-rata existed, so adding this moves nobody's pay.
 *
 * Counted in calendar days, not working days, and then the working-day
 * denominator is scaled by the same proportion. Counting employment in working
 * days instead would mean a joiner's first month depended on which weekday they
 * started, and two people joining the same week could be paid differently for
 * the same days.
 *
 * Both ends are INCLUSIVE. Someone who joins on the 15th is paid for the 15th,
 * and someone whose last day is the 20th is paid for the 20th. Off-by-one here
 * is a day of someone's pay, so it is stated rather than inferred.
 */
export function employedDaysInMonth(month, { joinedOn = null, leftOn = null } = {}) {
  const dim = daysInMonth(month);
  const [y, m] = String(month).split("-").map(Number);
  if (!y || !m) return dim;

  // Compared as plain dates, never as instants — see the note on the columns in
  // schema.prisma. A Date built from a @db.Date column is midnight UTC, so the
  // day is read in UTC to match.
  const dayOf = (d) => {
    if (!d) return null;
  ...
  const asIndex = (p) => (p ? p.y * 10000 + p.m * 100 + p.d : null);

  const monthStart = y * 10000 + m * 100 + 1;
  const monthEnd = y * 10000 + m * 100 + dim;

  const joined = asIndex(dayOf(joinedOn));
  const left = asIndex(dayOf(leftOn));

  // Joined after this month ended, or left before it began: not on the payroll
  // at all. Zero, not a full month — the caller decides what to do with that.
  if (joined !== null && joined > monthEnd) return 0;
  if (left !== null && left < monthStart) return 0;

  const firstDay = joined !== null && joined > monthStart ? (joined % 100) : 1;
  const lastDay = left !== null && left < monthEnd ? (left % 100) : dim;
...
export function payslip({
  ...
  const fullMonthGross = structure?.monthlyGross ?? 0;

  // ── part months ──────────────────────────────────────────────────────────
  // A joiner or a leaver is paid for the days they were actually on the
  // payroll: monthly gross ÷ calendar days in the month × days employed. The
  // working-day denominator is scaled by the same proportion, so a day of
  // unpaid absence inside a part month is still worth one day — not more,
  // because the denominator shrank with the numerator.
  //
  // With no dates set, employed === calendar days, the factor is 1, and every
  // line below is arithmetically identical to what it was. Nobody's pay moves
  // until somebody fills in a date.
  const dim = daysInMonth(month);
  const employedDays = employedDaysInMonth(month, {
    joinedOn: user?.joinedOn ?? null,
    leftOn: user?.leftOn ?? null,
  });
  const partMonth = employedDays < dim;
  const monthlyGross = partMonth
    ? Math.round((fullMonthGross * employedDays) / Math.max(1, dim))
    : fullMonthGross;

  const fullScheduledDays = scheduledWorkingDays({ month, workingDaysPerWeek: dpw });
  const scheduledDays = partMonth
    ? Math.max(1, Math.round((fullScheduledDays * employedDays) / Math.max(1, dim)))
    : fullScheduledDays;
  ...
  return {
    ...
    name: user?.name,
    month,
    // The pro-rated figure when it is a part month — this is what the payslip
    // is actually built from. contractedMonthlyGross below is the full salary,
    // kept separate so the two are never confused on screen.
    monthlyGross,
    contractedMonthlyGross: fullMonthGross,
    employedDays,
    calendarDays: dim,
    partMonth,
    ...
    payBasis: (() => {
      // Said in words so nobody has to reverse-engineer the number, and so a
      // part month explains itself on the payslip rather than looking like a
      // mistake somebody has to come and ask about.
      const prorata = partMonth
        ? `${employedDays} of ${dim} days on the payroll (₹${(fullMonthGross || 0).toLocaleString("en-IN")} × ${employedDays}/${dim} = ₹${(monthlyGross || 0).toLocaleString("en-IN")})`
        : null;
      const absence = summary.unpaidHalves
        ? `less ${summary.unpaidDays} unpaid day${summary.unpaidDays === 1 ? "" : "s"} at ₹${dayValue.toLocaleString("en-IN")} (÷ ${scheduledDays} scheduled working days)`
        : null;
      if (prorata && absence) return `${prorata}, ${absence}`;
      if (prorata) return `${prorata} — no unpaid absence`;
      if (absence) return `Full salary ${absence}`;
      return "Full monthly salary — no unpaid absence";
    })(),
