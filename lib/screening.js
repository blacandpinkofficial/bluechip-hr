...
export function screen(candidate = {}, requirement = {}) {
  ...
  // `ceiling > 0`, not `ceiling != null`. A ceiling of zero means nobody has
  // set the pay band — it is not an offer of nothing — and dividing by it gave
  // "Expecting Infinity% above the ceiling", which then cleared the >= 25 test
  // and blocked every single candidate on that opening.
  if (want != null && ceiling != null && ceiling > 0 && want > ceiling) {
  ...
  } else if (want != null && ceiling === 0) {
    add(unknowns, "money", "No pay band set on this opening",
      "Check what it actually pays before quoting anything.");
  }
